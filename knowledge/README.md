...
`index-validate` recomputes the catalogue from the working tree and compares all generated bytes. After staging the complete intended source tree and regenerated outputs, validate the exact prospective commit with:

```sh
python3 tools/rl_conveyor.py index-validate --staged
```

The staged form fails if a catalogue input is absent from or differs in the Git index. Stage the intended source changes as well as all three generated files before running it. Generation publishes each changed output atomically, writes metadata last, and leaves byte-identical outputs untouched.
...
The current checkout is authoritative about catalogue freshness, not historical prose. Run `index-validate` for the working tree or `index-validate --staged` for the prospective commit. `index_metadata.json` records the generation result, but its presence alone cannot establish that indexed inputs have not changed.

A frozen handover may correctly say that catalogue maintenance was `stale/deferred` when that handover was created. Treat that wording as an as-of-freeze record. A later infrastructure refresh can make the live catalogue current without changing the frozen statement or any mathematical state.
