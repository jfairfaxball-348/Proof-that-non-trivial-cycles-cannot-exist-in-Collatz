...
A green flat-authority preflight is the shell-worker equivalent of the three ordinary startup commands for that incoming session. It must fail closed unless all of the following hold: `authoritative/START_HERE.md` explicitly declares one incoming RL and names exactly one target for that RL; the predecessor kickoff, closeout-verification record, and frozen `sessions/RL<predecessor>/` exist; the local HEAD equals the live remote/default-branch HEAD; `authoritative/` exactly matches its committed Git tree; no current ZIP/sidecar/bundle transport is present; and the current portable verifier **and** red-team commands explicitly listed by `START_HERE.md` pass in isolated Python. Older inherited ZIPs are permitted only when each has its matching outer sidecar, the checksum passes, and its RL number predates the completed predecessor. Historical target files are permitted only when the entry point identifies the unique current target and no other current or future target exists. The helper records the committed authoritative tree identity and initializes/reuses the ordinary ignored `.rl-work/RL<incoming>/` checkpoint.

This fallback is not permission to bypass a genuine checksum, verifier, red-team, scope, mathematical, concurrency, or authority failure. If a current packaged transport is present, use `rl_conveyor.py`; if the flat preflight fails, enter stop-and-repair.
