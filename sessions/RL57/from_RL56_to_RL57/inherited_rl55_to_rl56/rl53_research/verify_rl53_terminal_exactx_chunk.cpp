...
int main(int ac,char**av){if(ac<8){std::cerr<<"usage Z X Y PREC SPLIT CHUNK NCHUNKS\n";return 2;}int Z=atoi(av[1]);XMAX=atoi(av[2]);YMAX=atoi(av[3]);int P=atoi(av[4]),SPLIT=atoi(av[5]),CH=atoi(av[6]),NC=atoi(av[7]);if(CH<0||CH>=NC)return 2;P3.resize(std::max(P+1,YMAX+3));P3[0]=1;for(size_t i=1;i<P3.size();i++)P3[i]=P3[i-1]*3;cpp_int A("123139092617126647266"),E("77692117359936589403"),K=A-E-Z+3;u128 q=pow2(K,P3[P])+1;if(q>=P3[P])q-=P3[P];std::vector<Task>all;collect(q,P,0,0,0,SPLIT,all);if(collect_amb){std::cerr<<"ambiguity in collector\n";return 1;}std::vector<Task>ts;for(size_t i=0;i<all.size();++i)if((int)(i%NC)==CH)ts.push_back(all[i]);auto t0=std::chrono::steady_clock::now();int best=collected_done;
#pragma omp parallel for schedule(dynamic,1) reduction(max:best)
for(int i=0;i<(int)ts.size();++i){int z=dfs(ts[i].q,ts[i].p,ts[i].x,ts[i].y);if(z>=900000)best=1000000;else best=std::max(best,ts[i].prefix+z);}double sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();std::cout<<"chunk Z="<<Z<<" X="<<XMAX<<" Y="<<YMAX<<" split="<<SPLIT<<" chunk="<<CH<<"/"<<NC<<" all_tasks="<<all.size()<<" chunk_tasks="<<ts.size()<<" baseline="<<collected_done<<" max="<<best<<" sec="<<sec<<"\n";return best>=900000?1:0;}
