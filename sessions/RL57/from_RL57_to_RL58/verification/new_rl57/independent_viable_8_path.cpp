...
static bool rec(int i,int p,int rem,cpp_int const& Znum,int d,long long J){
    ...
    if(rem==0){
        std::cout << "COUNTEREXAMPLE PREFIX FOUND i="<<i<<" p="<<p<<" d="<<d<<" J="<<J<<" K="<<ks.K<<" Z="<< Znum.convert_to<long double>()/den.convert_to<long double>() <<"\n";
        std::cout << "XPATH "<<path<<"\n";
        return true;
    }
    ...
    if(cpp_int(30)*P2[i] <= cpp_int(17)*den){
        Step s=step(d,J,0);
        if(s.ok){ path.push_back('0'); if(rec(i+1,p,rem-1,Znum+P2[i],s.d,s.J)) return true; path.pop_back(); }
    }
    // x=1 branch.
    Step s=step(d,J,1);
    if(s.ok){ path.push_back('1'); if(rec(i+1,p+1,rem,cpp_int(3)*Znum,s.d,s.J)) return true; path.pop_back(); }
    return false;
}
...
int main(){
    ...
    std::cout << "INDEPENDENT_CPP TARGET 8 hit "<<(hit?"True":"False")
