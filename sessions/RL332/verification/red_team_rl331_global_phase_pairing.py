#!/usr/bin/env python3
"""Independent RL331 red team for self-consistent ownership and phase gain."""
...
BOOT=32_551_214_209; CAP=BOOT-1; RHO=60; T=L-RHO
...
print("RL331_OWNERSHIP_RECONSTRUCTION_GREEN")
...
print("old_consumer_rhs",float(rhs))

# Independent phase audit.  Use Kosaraju rather than the verifier's Tarjan
# traversal, and derive the two condensation constants afresh.
zero=[]; forward=defaultdict(list); reverse=defaultdict(list)
for s,t,z,k in edges:
    slack=potential[t]-potential[s]-(2*z-43*k)
    assert slack>=0
    if slack==0:
        zero.append((s,t,z,k)); forward[s].append(t); reverse[t].append(s)
...
for v in reversed(order):
    if v not in seen:
        c=[]; dfs2(v,c); comps.append(c)
which={v:i for i,c in enumerate(comps) for v in c}
cyclic={i for i,c in enumerate(comps) if len(c)>1 or any(s==t and which[s]==i for s,t,_,_ in zero)}
...
for s,t,z,k in zero:
    if which[s]==which[t] and which[s] in cyclic:
        assert k==1 and ((s in low)!=(t in low))
        inside[s].append((t,z+k))
for s,outs in inside.items():
    if s in low:
        for mid,r1 in outs:
            for target,r2 in inside[mid]:
                assert target in low and r1+r2==45

dag=defaultdict(list); indegree=[0]*len(comps); arcs=set()
for s,t,z,k in zero:
    c,d=which[s],which[t]
    if c!=d:
        dag[c].append((d,k))
        if (c,d) not in arcs: arcs.add((c,d)); indegree[d]+=1
queue=[c for c,x in enumerate(indegree) if x==0]; topo=[]
while queue:
...
external=[0]*len(comps); segments=[int(c in cyclic) for c in range(len(comps))]
for c in topo:
    for d,k in dag[c]:
        external[d]=max(external[d],external[c]+k)
        segments[d]=max(segments[d],segments[c]+int(d in cyclic))
assert max(external)==16 and max(segments)==16

step=(45*A)%L
u,v=L,step; digits=[]
while v:
    digits.append(u//v); u,v=v,u%v
assert sum(digits)==172
variation=2; discrepancy=variation*sum(digits)
gamma=1/log2_hi-1

@lru_cache(None)
def residue_lower(count):
    a1=count*(count+1)//2
    a2=count*(count+1)*(2*count+1)//6
    a3=a1*a1
    a4=count*(count+1)*(2*count+1)*(3*count*count+3*count-1)//30
    return (Fraction(count)+log2_lo*a1/L+log2_lo**2*a2/(2*L**2)+
            log2_lo**3*a3/(6*L**3)+log2_lo**4*a4/(24*L**4))

def structural(kbase,h,s0):
    slack=45*h+s0; actual=kbase+h
    anchors2=actual-35*slack-32
    return Fraction(actual)+gamma*anchors2/2-discrepancy*16*(slack+1)

def min_gain(kbase,s0):
    baseweight=residue_lower(kbase)
    slope=structural(kbase,1,s0)-structural(kbase,0,s0)
    crossing=(structural(kbase,0,s0)-baseweight)/(1-slope)
    q=max(0,crossing.numerator//crossing.denominator)
    return min(max(Fraction(h),structural(kbase,h,s0)-baseweight) for h in (q,q+1))

gain60=min_gain(K,3)
assert gain60==5072
for kbase in range(K-79,K+1):
    for s0 in range(45): assert min_gain(kbase,s0)>=5071

def omit(last):
    return sum((Fraction(1<<((A*r)//L),3**r) for r in range(1,last+1)),Fraction())/lam
def ordinary_rhs(rho):
    kbase=(2*(L-rho+1)-boundary+44)//45
    return 1+(full-omit(rho-1))/3-residue_lower(kbase)/(12*lam)

phase_rhs60=1+ideal-(weighted+gain60)/(12*lam)
assert phase_rhs60<BOOT
assert ordinary_rhs(61)-Fraction(5071,12)/lam<BOOT
assert ordinary_rhs(1846)<BOOT
assert rho59<BOOT
print("RL331_GLOBAL_PHASE_PAIRING_RED_TEAM_GREEN")
print("step45_cf_digit_sum_discrepancy",step,sum(digits),discrepancy)
print("condensation_bounds",max(external),max(segments))
print("uniform_gains",gain60,5071)
print("phase_rhs60",float(phase_rhs60),"cap",CAP)
