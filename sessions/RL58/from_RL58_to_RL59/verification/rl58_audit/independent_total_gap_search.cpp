...
static const cpp_rational TARGET = cpp_rational(77)/10;
...
static bool viable(State const& s, TermCap *tc_out=nullptr){
    ...
    if(s.Z + cpp_rational(s.rem)*CAP <= TARGET){ ++prTarget; return false; }
    need2(s.rem);
    if(s.Z + cpp_rational(P2[s.rem]-1)*g <= TARGET){ ++prTarget; return false; }
    if(s.Z + psi_room <= TARGET){ ++prTarget; return false; }
...
static bool dominated_or_insert(State const& s){
    ...
    auto &v=memo[k];
    for(auto const& t:v){
        if(t.Z>=s.Z && t.D<=D){ ++prMemo; return true; }
    }
    v.erase(std::remove_if(v.begin(),v.end(),[&](Pt const&t){
        return t.Z<=s.Z && t.D>=D;
    }),v.end());
    v.push_back({s.Z,D});
...
int main(){
    ...
    std::cout << "RL58_GAP_TOTAL_AUDIT target=77/10 hit="<<(hit?1:0)
