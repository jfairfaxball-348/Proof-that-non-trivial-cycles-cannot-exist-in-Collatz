...
LAST=2921801521
NEXT=2921801523
EXPECTED_COUNT=13396
MU=Fraction(1,7000001)
LAM=Fraction(4000000,52500238500033)
SHORT=15000053
corners=[
    ('A',0,7000001),
    ('B',5000031,4350001),
...
def top(k,l=LAM,m=MU):
    ...
    w=l*D+m*R; bad=w.numerator//w.denominator; good=b-bad
    cap=D//(C-SHORT)
...
assert start_margins=={'AC':187826246,'CD':184711488,'DE':-231729553},start_margins

cnt=0; worst=None; worst_k=None
for k in range(K0,LAST+1,2):
    st=top(k); assert st[-1]>0,(k,st)
    D,R=st[1],st[2]; ac=LAM*D+MU*R
...
assert worst==10473,worst
assert top(LAST)==(123139091657857127643,17564442249346917,11081960984517759,2921374196,427324,416851,10473)
assert top(NEXT)==(123139091657857127496,17564526551210320,11082014173071665,2921388218,413304,416853,-3549)
D,R=top(NEXT)[1:3]; ac=LAM*D+MU*R
assert all(ac<=l*D+m*R for _,l,m in feasible)
print('RL97 coupled interval verifier: PASS')
